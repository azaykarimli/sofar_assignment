...
class BehaviorNavigator(Node):
    ...
    def setInitialPose(self, initial_pose):
        ...
        return self.setinitialpose
    
    def goThroughPoses(self, poses, behavior_tree=''):
        """send a navthroughpose action request"""
        ...
        send_goal_future = self.nav_through_poses_client.send_goal_async(goal_msg, self.get_feedbackCallback)
    ...
    def goToPose(self, pose, behavior_tree= ''):
        """send navtopose action request"""
        self.debug("waiting for 'NavToPose' action server")
        ...
        if not self.goal_handle.accepted:
            self.__error('goal to' + str(pose.pose.position.x) + '' + str(pose.pose.position.y) + 'was rejected')
            return False
    ...
    def followWaypoints(self, poses):
        """send a FollowWayPoint action request"""
        ...
        send_goal_future = self.follow_waypoints_client.send_goal_async(goal_msg,self._feedbackCallback)
        rclpy.spin_until_future_complete(self, send_goal_future)
        self.goal_handle = send_goal_future.result()

        if not self.goal_handle.accepted:
            self._error(f'following {len(goal_msg.poses)} waypoints request was rejected')
            return False
